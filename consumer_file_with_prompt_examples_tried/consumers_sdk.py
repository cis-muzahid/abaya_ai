...
class ChatConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
        ...
        print("Data:", data)
        states.USER_INPUT.append(message)
        ...
        if base64_image:
            ...
            if reply_to_message.get('previous_user_input'):
                ...
                try:
                    ...
                    breakpoint()
                    edited_image_url_via_old_res = self.edit_image_with_previous_user_input(new_prompt=message, previous_response=self.USER_INPUT)
    ...
    def generate_dalle_image(self, prompt):
        # prompt = f"""
        #     Your task is to generate a single hyper-realistic abaya image with the following requirements:
        #     1. The image must be in PNG format.
        #     2. The image must be full-length.
        #     3. The image must include an RGBA color profile.
    ...
    def edit_image_with_previous_user_input(self, new_prompt, previous_response):
        """
        Combines a provided image with a prompt to generate a new image using DALL-E.
        """
        print("Editable prompt:", new_prompt)
        final_prompt = " + ".join(f"'{item}'" for item in previous_response)
        breakpoint()
        prompt = """
        1. Your job is to do updation in the image that is previously created by previous prompt {}. 2. Do not change the whole image like keep the same dress, pattern, size, dimentions, base color, Just update whatever is passed in the promt: """+ new_prompt
        final_prompt = new_prompt + previous_response
        try:
            print("Editing the image..")    
            response = client.images.generate(
                model="dall-e-3",
